...
| `demand-intelligence/opportunity-schema.json`, filtered to `classification == "Convert into Product Idea"` | Demand Intelligence's own decision tree, triggered when `recurrencePattern: "product"` was set at ingestion | The classification is already made; the runtime consumes the label, exactly as Content Director consumes `Convert into Content` |
