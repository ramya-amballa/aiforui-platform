"""
Generic RSS/Atom feed fetcher — real, dependency-free (urllib +
the standard library's xml.etree.ElementTree), no third-party feed
parser. Handles both RSS 2.0 (<rss><channel><item>) and Atom
(<feed><entry>) since sources in market-intelligence-classification-model.md
use either. A source with no feed URL configured is skipped by the
caller (monitor.py), not by this module — this module only knows how
to fetch and parse a URL it's given.
"""
...
def fetch_feed_entries(url, timeout=15):
    ...
    rest of the run."""
    request = urllib.request.Request(url, headers={"User-Agent": "AOS-MarketIntelligence/1.0"})
    try:
