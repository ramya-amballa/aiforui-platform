...
The 20 sources Demand Intelligence (formerly Opportunity Hunter)
monitors, grouped by channel. Each source produces entries in
`opportunity-schema.json` tagged with its `source` and
`sourceCategory`, so downstream filtering (by channel, by domain)
never requires re-reading raw listings.

## Demand Signal (the primary discovery mode)
...
every channel below, which all answer "who has advertised a vacancy."
