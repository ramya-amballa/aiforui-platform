#!/usr/bin/env python3
"""
Demand Intelligence — Daily Ingestion Workflow (execution mode)
...
# 08-Revenue-Hunter/lead-scoring.md, reusing Demand Intelligence's
...
def route_to_crm(opportunity, crm_data):
    ...
    outreach_entry = {
        ...
        "summary": f"Demand Intelligence logged: {opportunity['title']} ({opportunity['classification']})",
    }
...
def compute_relevance_for_record(record):
    ...
    collectors/demand_signals.py's records (source == "Demand Signal")
    are exempted the same way, for a different reason: they've already
    passed a stricter, more targeted check than any keyword rule here
    could apply — a Claude API read of the actual article, only
    proceeding at "high" confidence that a named organisation is
    adopting an AI tool at scale (see that module's own docstring).
    Re-running keyword-based relevance scoring over that already-
    validated judgement would only risk a false rejection (the article
    text is about the vendor's product, not phrased like a job
    posting), not add any real signal."""
    if record.get("source") in ("Website", "Demand Signal"):
