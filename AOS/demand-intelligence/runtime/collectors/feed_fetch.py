...
05-Market-Intelligence/runtime/feeds.py's fetch_feed_entries() (same
dependency-free urllib + xml.etree.ElementTree approach, no
third-party feed parser), so demand_signals.py doesn't re-derive a
second RSS/Atom parser. See that file for the original.
"""
...
def fetch_feed_entries(url, timeout=15):
    ...
    rest of the run."""
    request = urllib.request.Request(url, headers={"User-Agent": "AOS-DemandIntelligence/1.0"})
    try:
