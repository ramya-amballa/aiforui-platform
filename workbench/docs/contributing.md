...
2. Choose an `id` following the type's prefix convention (e.g. `pattern-my-new-pattern`) — lowercase, kebab-case, globally unique.
3. Fill in every field required by the schema (see `/docs/schemas.md`), plus at least one citation if your object's `confidence` will be `Verified` or `Reviewed`.
4. Add `relationships` connecting your new object into the graph — an object with none is rejected as an orphan node (see `/docs/relationship-model.md`). If you're not sure what it should connect to, `RELATED_TO` a relevant `decision` is a reasonable starting point.
5. Set `confidence` honestly — new community submissions should generally start at `Draft` or `Community`, not `Reviewed`/`Verified` (see `/docs/confidence-model.md`); a maintainer promotes it after review.
6. Run `npm run validate` from `/workbench` and fix anything it flags before opening a pull request.
7. Optionally, add yourself to `contributors`.
