...
**4. Practitioner Assets.** The graph shouldn't only be something you read — it should be something you leave with. Concrete forms this takes: **Governance Playbooks** (executive summary, common decisions, historical incidents, patterns, framework mapping, evidence checklist, board questions, and an audit checklist, assembled per practitioner topic — e.g. a Customer-Facing AI Governance Playbook), **Architecture Review Packs** (a decision checklist, required evidence, controls, common failure modes, and historical incidents for a specific system category — e.g. a Generative AI Architecture Review), **Board Brief Packs** (e.g. AI Hiring Systems), **Risk Assessment Templates** (e.g. Vendor AI Due Diligence, generated from the Decisions/Patterns/Controls/Evidence already in the graph), and **Audit Worksheets** — every one of them assembled deterministically from canonical fields, never generated freehand. This is the next concrete step past the Explorer, and likely the point where people start bookmarking the Workbench rather than just visiting it once. See `FOUNDATION_COMPLETE.md` for why this work can proceed without reopening anything the foundation freezes. The first Governance Playbook — `playbooks/customer-facing-ai-governance.md` — now exists as the reference implementation every future one is held to; it deliberately precedes any generator or template infrastructure, to prove the format is right before automating it.

**5. Community Trust.** Before this project opens itself to outside contributors, it needs the policy scaffolding that makes contribution safe to accept. As of Phase 4, that scaffolding exists: [`EDITORIAL_POLICY.md`](EDITORIAL_POLICY.md), [`CITATION_POLICY.md`](CITATION_POLICY.md), [`VERSION_POLICY.md`](VERSION_POLICY.md), [`REVIEW_PROCESS.md`](REVIEW_PROCESS.md), [`CONFLICT_OF_INTEREST.md`](CONFLICT_OF_INTEREST.md), [`METHODOLOGY.md`](METHODOLOGY.md), and [`GOVERNANCE_CHARTER.md`](GOVERNANCE_CHARTER.md), alongside the Contribution Guide (`docs/contributing.md`) and Release Notes (`docs/releases/`) that already existed. What Phase 4 did not do — deliberately — is actually open the project to outside contributors; that remains a future, separate decision. Trust has to be built before scale is invited in, not the other way around, and this workstream is the "built" half of that sentence, not the "invited in" half.
