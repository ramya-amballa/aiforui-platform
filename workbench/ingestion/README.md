...
- `src/promote.ts` — the only path from a draft into `/data/incidents`. It refuses to run unless `human_review.status` is `"approved"`, a `reviewer` and `confidence_assigned` are recorded, and the resulting object passes `incident.schema.json`. It never invents relationships — those are added by hand afterwards.
